...
def save_reminders(data: dict):
    """Save reminders to JSON file atomically."""
    try:
        # Write to temp file first
        fd, temp_path = tempfile.mkstemp(dir=os.path.dirname(DATA_FILE), text=True)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Welcome message."""
    # Save user on start so they can be found by others
    ...
    await update.message.reply_text(
        ...
        "📌 Commands:\n"
        "1. /newreminder - Create a reminder\n"
        "2. /listreminders - View your reminders\n"
        "3. /editreminder - Edit a reminder\n"
        "4. /deletereminder - Delete a reminder\n"
        "5. /getid - Get current chat ID\n"
        "6. /cancel - Cancel current operation"
    )
